...
d2 = {"eng" : 90, "sci": 95}
print({**d, **d2}) #합치는 방법 언패킹하고 합쳐버리기 update() 동일한 결과 이거는 새로운 딕셔너리 생성, 업데이트는 기존 딕셔너리 차이
...
result = {data : key for key, data in result.items()}
print(result)

# 점수 90 이상만 필터링하기

scores = {"국어": 69, "영어": 93, "수학": 92, "과학": 68}
...
stock1 = {"연필": 10, "지우개": 5, "노트": 3}        # 기존 재고
stock2 = {"연필": 10, "지우개": 5, "노트": 3}        # 기존 재고
incoming = {"지우개": 4, "노트": 7, "볼펜": 12}     # 입고 내역

# 1) 클래식 for
for item, qty in incoming.items():
    stock1[item] = stock1.get(item, 0) + qty

print(stock1)  

# 2) dict 컴프리헨션
stock2.update({item: stock2.get(item, 0) + qty for item, qty in incoming.items()})

print(stock2)                                # ✅ {'연필': 10, '지우개': 9, '노트': 10, '볼펜': 12}